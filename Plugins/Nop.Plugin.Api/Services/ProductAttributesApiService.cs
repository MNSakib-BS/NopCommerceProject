...
namespace Nop.Plugin.Api.Services
{
    public class ProductAttributesApiService : IProductAttributesApiService
    {
        ...
        public async Task<IList<ProductAttribute>> GetProductAttributesAsync(
        ...
        public async Task<int> GetProductAttributesCountAsync()
        {
            return GetProductAttributesQuery().Count();
        }

        async Task<ProductAttribute> IProductAttributesApiService.GetByIdAsync(int id)
        {
            if (id <= 0)
            {
                return null;
