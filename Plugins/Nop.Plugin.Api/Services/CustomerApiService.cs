...
namespace Nop.Plugin.Api.Services
{
    public class CustomerApiService : ICustomerApiService
    {
        ...
        private readonly IRepository<CustomerPicture> _customerPictureMappingRepository;
        ...
        public CustomerApiService(
            ...
            IRepository<CustomerPicture> customerPictureMappingRepository,
            ...
            IPictureService pictureService)
        {
            ...
            _customerPictureMappingRepository = customerPictureMappingRepository;
        ...
        public async Task<int> GetCustomersCountAsync()
        {
            return _customerRepository.Table.Count(customer => !customer.Deleted
                                                               && (customer.RegisteredInStoreId == 0 ||
                                                                   customer.RegisteredInStoreId == _storeContext.GetCurrentStoreAsync().Id));
        }
        ...
        public async Task<Dictionary<string, string>> GetFirstAndLastNameByCustomerIdAsync(int customerId)
        {
        ...
        public async Task<CustomerDto> GetCustomerByIdAsync(int id, bool showDeleted = false)
        {
            ...
            var pictureMapping =await GetCustomerPictureMappingAsync(customerDto.Id);

            if (pictureMapping != null)
            {
                var pictureBytes =(await _pictureService.GetPictureBinaryByPictureIdAsync(pictureMapping.PictureId))?.BinaryData;

                if (pictureBytes != null)
                {
                    customerDto.Photo = Convert.ToBase64String(pictureBytes);
                }                
            }
        ...
        public async Task<CustomerPicture> GetCustomerPictureMappingAsync(int customerId)
        {
            return _customerPictureMappingRepository.Table.FirstOrDefault(i => i.CustomerId == customerId);
        }
        ...
        private IQueryable<Customer> GetCustomersQuery(DateTime? createdAtMin = null, DateTime? createdAtMax = null, int sinceId = 0)
        {
            ...
            var excludeRoleIds = _customerRoleRepository.Table.Where(i => i.Active && i.SystemName == Core.Domain.Customers.NopCustomerDefaults.GuestsRoleName)
                                                               .Select(i => i.Id).Distinct().ToArray();

            var excludeCustomerIds = _customerCustomerRoleMappingRepository.Table.Where(i => excludeRoleIds.Contains(i.CustomerRoleId))
                                                                                  .Select(i => i.CustomerId).ToArray();

            query = query.Where(customer =>
                                    !excludeCustomerIds.Contains(customer.Id)
                                    && (customer.RegisteredInStoreId == 0 || customer.RegisteredInStoreId == _storeContext.GetCurrentStoreAsync().Id));
        ...
        {
            // Get the default language id for the current store.
            var defaultLanguageId =  _storeContext.GetCurrentStoreAsync().DefaultLanguageId;
            ...
            if (defaultLanguageId == 0)
            {
                ...
                var storeLanguages = allLanguages.Where(l =>
                                                            _storeMappingService.Authorize(l, _storeContext.GetCurrentStoreAsync().Id)).ToList();
        ...
        private IEnumerable<string> GetAllNewsletterCustomersEmails()
        {
            return _cacheManager.Get(Constants.Configurations.NEWSLETTER_SUBSCRIBERS_KEY, () =>
            {
                IEnumerable<string> subscriberEmails = (from nls in _subscriptionRepository.Table
                                                        where nls.StoreId == _storeContext.GetCurrentStoreAsync().Id
                                                              && nls.Active
