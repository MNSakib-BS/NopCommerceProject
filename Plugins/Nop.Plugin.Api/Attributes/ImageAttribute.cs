...
namespace Nop.Plugin.Api.Attributes
{
    public class ImageValidationAttribute : BaseValidationAttribute
    {
        ...
        public override void Validate(object instance)
        {
            ...
            if (imageSrcSet || imageAttachmentSet)
            {
                ...
                if (_errors.Count == 0)
                {
                    ...
                    ValidatePictureByteArray(imageBytes, mimeType);
                }
        ...
        private void ValidatePictureByteArray(byte[] imageBytes, string mimeType)
        {
            if (imageBytes != null)
            {
                try
                {
                    imageBytes = _pictureService.ValidatePictureAsync(imageBytes, mimeType);
                }
                catch (Exception ex)
                {
