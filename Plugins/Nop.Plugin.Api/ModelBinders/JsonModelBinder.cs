...
namespace Nop.Plugin.Api.ModelBinders
{
    public class JsonModelBinder<T> : IModelBinder where T : class, new()
    {
        ...
        private async Task<Dictionary<string, object>> GetPropertyValuePairs(ModelBindingContext bindingContext)
        {
            ...
            if (bindingContext.ModelState.IsValid)
            {
                try
                {
                    ...
                    var rootPropertyName =await _jsonHelper.GetRootPropertyNameAsync<T>();
