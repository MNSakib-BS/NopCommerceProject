...
{
    public class ProductsController : SecureBaseApiController
    {
        ...
        public async Task<IActionResult> GetProducts(ProductsParametersModel parameters)
        {
            ...
            var allProducts = await _productApiService.GetProductsAsync(parameters.Ids, parameters.CreatedAtMin, parameters.CreatedAtMax, parameters.UpdatedAtMin,
            ...
            Delta<ProductDto> productDelta)
        {
            ...
            var product = await _productApiService.GetProductByIdAsync(productDelta.Dto.Id);
        ...
        private async Task UpdateProductPictures(Product entityToUpdate, List<ImageMappingDto> setPictures)
        {
        ...
        private async Task UpdateProductAttributes(Product entityToUpdate, Delta<ProductDto> productDtoDelta)
        {
        ...
        private async Task UpdateProductAttributeValues(ProductAttributeMappingDto productAttributeMappingDto, Delta<ProductDto> productDtoDelta)
        {
            ...
            var unusedProductAttributeValues =
                _productAttributeService.GetProductAttributeValuesAsync(productAttributeMappingDto.Id).Where(x => !toBeUpdatedIds.Contains(x.Id)).ToList();

            foreach (var unusedProductAttributeValue in unusedProductAttributeValues)
            {
                _productAttributeService.DeleteProductAttributeValueAsync(unusedProductAttributeValue);
            }
        ...
        private async Task UpdateProductTags(Product product, IReadOnlyCollection<string> productTags)
        {
            if (productTags == null)
            {
                return;
            ...
            foreach (var productTagName in productTags)
            {
                ...
                var productTag2 = await _productTagService.GetProductTagByNameAsync(productTagName);
                if (productTag2 == null)
                {
                    //add new product tag
        ...
        private async Task UpdateDiscountMappings(Product product, List<int> passedDiscountIds)
        {
        ...
        private async Task UpdateAssociatedProducts(Product product, List<int> passedAssociatedProductIds)
        {
