...
namespace Nop.Plugin.Api.Validators
{
    public abstract class BaseDtoValidator<T> : AbstractValidator<T> where T : BaseDto, new()
    {
        ...
        private async Task< Dictionary<string, object>> GetRequestJsonDictionaryDictionaryFromHttpContext()
        {
            var requestJsonDictionary =await JsonHelper.GetRequestJsonDictionaryFromStreamAsync(HttpContextAccessor.HttpContext.Request.Body, true);
            var rootPropertyName =await JsonHelper.GetRootPropertyNameAsync<T>();
