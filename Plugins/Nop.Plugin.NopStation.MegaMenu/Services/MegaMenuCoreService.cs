...
namespace Nop.Plugin.NopStation.MegaMenu.Services
{
    public class MegaMenuCoreService : IMegaMenuCoreService
    {
        ...
        public virtual async Task<IList<Category>> GetCategoriesByIdsAsync(int storeId = 0, List<int> selectedIds = null, int pageSize = int.MaxValue, bool showHidden = false, int storeTypeId = 0)
        {
            var query = _categoryRepository.Table;

            if (storeTypeId > 0)
                query = query.Where(p => p.StoreTypeId == storeTypeId);
            ...
            query = query.OrderBy(m => m.DisplayOrder).ThenBy(p => p.Name);
