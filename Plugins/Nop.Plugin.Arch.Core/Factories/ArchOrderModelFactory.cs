...
/// </summary>
public partial class ArchOrderModelFactory : IOrderModelFactory
{
    #region Fields
    ...
    public ArchOrderModelFactory(AddressSettings addressSettings,
    ...
    {
        return PrepareCustomerOrderListModel(new CustomerOrderPagingFilteringModel());
    }

    private OrderCustomerWalletTransaction GetCashback(Order order)
    {
        var archSettings = _settingService.LoadSetting<ArchApiSettings>(order.StoreId);

        var customerWalletTransaction = _customerWalletService.GetAllCustomerWalletTransactions(
                transactionTrackingNumber: order.TransactionTrackingNumber,
                transactionType: WalletTransactionType.Credit,
                verifiedToArch: true,
                storeId: order.StoreId)
            .FirstOrDefault(i => i.CashbackStatusId == (int)CashbackStatus.Pending ||
                                 i.CashbackStatusId == (int)CashbackStatus.Voided);

        if (customerWalletTransaction != null)
        {
            var cashbackDaysValidFor =
                Math.Round(archSettings.ExpireCashbackAfterDays - DateTime.UtcNow.Subtract(customerWalletTransaction.CreatedDateUtc).TotalDays, 1);

            var cashback = new OrderCustomerWalletTransaction
            {
                Id = customerWalletTransaction.Id,
                Amount = _priceFormatter.FormatPrice(customerWalletTransaction.Amount, true, false),
                DaysValidFor = cashbackDaysValidFor < 0 ? 0 : cashbackDaysValidFor,
                CashbackStatus = _localizationService.GetLocalizedEnum(customerWalletTransaction.CashbackStatus),
                ModifiedDate = _dateTimeHelper.ConvertToUserTime(customerWalletTransaction.ModifiedDateUtc, DateTimeKind.Utc),
                LastModifiedTicks = customerWalletTransaction.ModifiedDateUtc.Ticks.ToString(),
                CashBackAmount = customerWalletTransaction.Amount,
                Status = customerWalletTransaction.CashbackStatus
            };
    ...
    public override string GetArchInvoiceRefundTotal(Order order)
    {
        string refundTotal = "0.00";
        decimal refundAmount = 0;
        var archInvoiceItemList = new List<OrderDetailsModel.ArchInvoiceItemModel>();
        var archInvoiceItems = _archApi.EnsureInvoice(order.TransactionTrackingNumber, order.StoreId);

        foreach (var archInvoiceItem in archInvoiceItems)
        {
            var product = _productService.GetProductsByCodes(null, archInvoiceItem.ProductCode, order.StoreId).FirstOrDefault();
            if (product == null)
                continue;

            var invoiceItemModel = new OrderDetailsModel.ArchInvoiceItemModel
            {
                Id = archInvoiceItem.Id,
                DebtorNumber = archInvoiceItem.DebtorNumber,
                LoyaltyCardNumber = archInvoiceItem.LoyaltyCardNumber,
                OrderNumber = archInvoiceItem.OrderNumber,
                PackQuantity = Convert.ToInt32(archInvoiceItem.PackQuantity),
                PackSellingPriceIncl = _priceFormatter.FormatPrice(archInvoiceItem.PackSellingPriceIncl, true, false),
                PackSize = archInvoiceItem.PackSize.ToString("0.####"),
                ProductCode = archInvoiceItem.ProductCode,
                ProductId = product.Id,
                ProductName = _hasMultipleLanguages ? _localizationService.GetLocalized(product, x => x.Name) : product.Name,
                ProductSeName = _urlRecordService.GetSeName(product),
                Quantity = Convert.ToInt32(archInvoiceItem.Quantity),
                TaxRate = _priceFormatter.FormatPrice(archInvoiceItem.TaxRate, true, false),
                TotalLineDiscount = _priceFormatter.FormatPrice(archInvoiceItem.TotalLineDiscount, true, false),
                TotalLineIncl = _priceFormatter.FormatPrice(archInvoiceItem.TotalLineIncl, true, false),
                TransactionTrackingNumber = archInvoiceItem.TransactionTrackingNumber
            };
            ...
            invoiceItemModel.Sku = _productService.FormatSku(product, orderItem?.AttributesXml);
            invoiceItemModel.VendorName = _vendorService.GetVendorById(product.VendorId)?.Name;

            archInvoiceItemList.Add(invoiceItemModel);
        }
        ...
        foreach (var orderItem in orderItems)
        {
            var product = _productService.GetArchProductById(orderItem.ProductId, order.StoreId);
            if (product == null)
                continue;
            var orderItemModel = new OrderDetailsModel.OrderItemModel
            {
            ...
            if (order.CustomerTaxDisplayType == TaxDisplayType.IncludingTax)
            {
                currentUnitPrice = orderItem.UnitPriceInclTax;
                currentTotalPrice = orderItem.PriceInclTax;

                //including tax
                var unitPriceInclTaxInCustomerCurrency = _currencyService.ConvertCurrency(orderItem.UnitPriceInclTax, order.CurrencyRate);
                orderItemModel.UnitPrice = _priceFormatter.FormatPrice(unitPriceInclTaxInCustomerCurrency, true, order.CustomerCurrencyCode, _workContext.WorkingLanguage.Id, true);

                var priceInclTaxInCustomerCurrency = _currencyService.ConvertCurrency(orderItem.PriceInclTax, order.CurrencyRate);
                orderItemModel.SubTotal = _priceFormatter.FormatPrice(priceInclTaxInCustomerCurrency, true, order.CustomerCurrencyCode, _workContext.WorkingLanguage.Id, true);
            }
            else
            {
                currentUnitPrice = orderItem.UnitPriceExclTax;
                currentTotalPrice = orderItem.PriceExclTax;

                //excluding tax
                var unitPriceExclTaxInCustomerCurrency = _currencyService.ConvertCurrency(orderItem.UnitPriceExclTax, order.CurrencyRate);
                orderItemModel.UnitPrice = _priceFormatter.FormatPrice(unitPriceExclTaxInCustomerCurrency, true, order.CustomerCurrencyCode, _workContext.WorkingLanguage.Id, false);

                var priceExclTaxInCustomerCurrency = _currencyService.ConvertCurrency(orderItem.PriceExclTax, order.CurrencyRate);
                orderItemModel.SubTotal = _priceFormatter.FormatPrice(priceExclTaxInCustomerCurrency, true, order.CustomerCurrencyCode, _workContext.WorkingLanguage.Id, false);
            }

            //refunded amount
            var refundedValue = CalculateArchInvoiceRefundTotal(archInvoiceItemList, orderItemModel, currentUnitPrice, currentTotalPrice);
            var refundedInCustomerCurrency = _currencyService.ConvertCurrency(refundedValue, order.CurrencyRate);
            orderItemModel.Refunded = _priceFormatter.FormatPrice(refundedInCustomerCurrency, true, order.CustomerCurrencyCode, _workContext.WorkingLanguage.Id, false);
            refundAmount += refundedValue;
        ...
        //Refunded Total
        var refundedAmountInCustomerCurrency = _currencyService.ConvertCurrency(refundAmount, order.CurrencyRate);
        return refundTotal = _priceFormatter.FormatPrice(refundedAmountInCustomerCurrency, true, order.CustomerCurrencyCode, false, _workContext.WorkingLanguage.Id);
    }


    public override decimal CalculateArchInvoiceRefundTotal(IList<OrderDetailsModel.ArchInvoiceItemModel> invoiceItems, OrderDetailsModel.OrderItemModel orderItem, decimal currentUnitPrice, decimal currentTotalPrice)
    {
        decimal refundValue = 0;

        if (invoiceItems != null && invoiceItems.Any() && orderItem != null)
        {
            var currentInvoiceItem = (from item in invoiceItems where item.ProductId == orderItem.ProductId select item).FirstOrDefault();
            if (currentInvoiceItem == null)
            {
                refundValue = currentTotalPrice;
            }
            else
            {
                var quantity = currentInvoiceItem.Quantity;
                if (quantity > 0)
                {
                    refundValue = currentTotalPrice - (quantity * currentUnitPrice);
                }
            }
        }
