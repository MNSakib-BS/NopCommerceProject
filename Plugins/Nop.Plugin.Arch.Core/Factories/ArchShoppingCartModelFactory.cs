...
/// </summary>
public partial class ArchShoppingCartModelFactory : IShoppingCartModelFactory
{
    #region Fields
    ...
    public ArchShoppingCartModelFactory(AddressSettings addressSettings,
    ...
    public override RecommendedListModel PrepareRecommendedListModel(RecommendedListModel model, WishlistModelPagingFilteringModel command)
    {
        if (model == null)
            throw new ArgumentNullException(nameof(model));

        var recommendedListService = EngineContext.Current.Resolve<IRecommendedListService>();
        var productModelFactory = EngineContext.Current.Resolve<IProductModelFactory>();

        var productMappings = recommendedListService.GetProductRecommendedListsByRecommendedListId(model.SelectedRecommendedListId);
        var productIds = productMappings.Select(i => i.ProductId).ToArray();

        //products
        var products = _productService.GetProductsByIds(productIds);
        model.Products = productModelFactory.PrepareProductOverviewModels(products).ToList();
        var recommendedListModelFactory = EngineContext.Current.Resolve<IRecommendedListModelFactory>();

        PrepareSortingOptions(model.PagingFilteringContext, command);

        var query = (from product in products
                     select recommendedListModelFactory.PrepareRecommendedListItemModel(product));
        ...
        switch ((ProductSortingEnum)model.PagingFilteringContext.OrderBy)
        {
            ...
            case ProductSortingEnum.NameAsc:
                query = query.OrderBy(i => i.ProductName);
                break;
            case ProductSortingEnum.NameDesc:
                query = query.OrderByDescending(i => i.ProductName);
                break;
        ...
        var pageIndex = model.PagingFilteringContext.PageIndex;
        var pageSize = model.PagingFilteringContext.PageSize;

        model.Items = new PagedList<RecommendedListModel.ShoppingCartItemModel>(query.ToList(), pageIndex, pageSize);
        model.PagingFilteringContext.LoadPagedList(model.Items);

        return model;
    }

    public override void PrepareSortingOptions(WishlistModelPagingFilteringModel pagingFilteringModel, WishlistModelPagingFilteringModel command)
    {
