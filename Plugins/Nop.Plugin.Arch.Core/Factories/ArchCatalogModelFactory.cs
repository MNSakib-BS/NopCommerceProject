...
public partial class ArchCatalogModelFactory : ICatalogModelFactory
{
    #region Fields
    ...
    public ArchCatalogModelFactory(BlogSettings blogSettings,
    ...
    public virtual async Task<CategoryModel> PrepareCategoryModelAsync(Category category, CatalogProductsCommand command)
    {
        ...
        if (model.BannerHeroPictureTopId > 0)
        {
            model.BannerHeroPictureTop = _pictureService.GetPictureUrl(category.BannerHeroPictureTopId);
        }

        if (model.BannerHeroPictureLowerId > 0)
        {
            model.BannerHeroPictureLower = _pictureService.GetPictureUrl(category.BannerHeroPictureLowerId);
        }
    ...
    {
        var history = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer, NopCustomerDefaults.SearchHistoryAttribute, _storeContext.ActiveStoreScopeConfiguration);
        var historyItems = history?.Split(";")?.ToList() ?? new List<string>();
        if (!historyItems.Contains(searchTerm.ToUpper()))
        {
            if (historyItems.Count == _catalogSettings.SearchHistoryCount)
                historyItems.RemoveAt(_catalogSettings.SearchHistoryCount - 1);

            historyItems.Insert(0, searchTerm.ToUpper());
        }
        else
        {
            historyItems.Remove(searchTerm);
            historyItems.Insert(0, searchTerm.ToUpper());
        }
        ...
        {
            if (i < historyItems.Count - 1)
            {
                newHistory += $"{historyItems[i]};";
            }
            else
            {
                newHistory += $"{historyItems[i]}";
        ...
        _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, NopCustomerDefaults.SearchHistoryAttribute, newHistory, _storeContext.ActiveStoreScopeConfiguration);
    }

    public override SearchSuggestionsModel GetSearchSuggestions()
    {
        var suggestions = new SearchSuggestionsModel();

        // Categories
        var categories = _categoryService.GetAllCategoriesDisplayedOnHomepage();
        suggestions.Categories = (from category in categories
                                  select new SearchBoxCategoryModel
                                  {
                                      Name = category.Name,
        ...
        // Search History
        var history = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer, NopCustomerDefaults.SearchHistoryAttribute, _storeContext.ActiveStoreScopeConfiguration);
        var historyList = history?.Split(";")?.ToList() ?? new List<string>();
        suggestions.PreviousSearches = (from item in historyList
                                        group item by item into g
                                        select g.Key).ToList();

        // Trending Searches
        var historyBackdate = _catalogSettings.TrendingSearchesBackdateHours > 0 ? (DateTime?)DateTime.UtcNow.AddHours(-1 * _catalogSettings.TrendingSearchesBackdateHours) : null;
        var historyAttributes = _genericAttributeService.GetAttributesByKey(NopCustomerDefaults.SearchHistoryAttribute, _storeContext.ActiveStoreScopeConfiguration, lastUpdatedFromUtc: historyBackdate);

        var historyCounts = new Dictionary<string, int>();
        for (int i = 0; i < historyAttributes.Count; i++)
        {
            GenericAttribute attribute = historyAttributes[i];
            history = attribute.Value;
            var searches = history?.Split(";")?.ToList() ?? new List<string>();
            for (int j = 0; j < searches.Count; j++)
            {
                string search = searches[j].ToUpper();
                if (historyCounts.ContainsKey(search))
                {
                    historyCounts[search]++;
                }
                else
                {
                    historyCounts.Add(search, 1);
                }
            }
        }

        historyCounts = historyCounts.OrderByDescending(x => x.Value).ToDictionary(x => x.Key, x => x.Value);

        suggestions.TrendingSearches = historyCounts.Select(i => i.Key).Take(_catalogSettings.TrendingSearchesCount).ToList();
    ...
    protected override CategorySimpleModel GetCategorySimpleModel(XElement elem)
    {
    ...
    #region PromotionGroups

    public override PromotionMegaMenuModel PreparePromotionMegaMenuGroupModel(PromotionGroup promotionGroup, CatalogPagingFilteringModel command)
    {
        //prepare banner picture model

        var pictureSize = _mediaSettings.PromotionGroupThumbPictureSize;
        var promotionGroupBannerPictureCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.PromotionGroupBannerPictureModelKey,
            promotionGroup, pictureSize, true, _workContext.WorkingLanguage,
            _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore);
        var bannerPictureModel = _staticCacheManager.Get(promotionGroupBannerPictureCacheKey, () =>
        {
            var picture = _pictureService.GetPictureById(promotionGroup.BannerPictureId);
            var pictureModel = new PictureModel
            {
                FullSizeImageUrl = _pictureService.GetPictureUrl(ref picture),
                ImageUrl = _pictureService.GetPictureUrl(ref picture, pictureSize),
                Title = string.Format(_localizationService.GetResource("Media.PromotionGroup.ImageLinkTitleFormat"), promotionGroup.Name),
                AlternateText = string.Format(_localizationService.GetResource("Media.PromotionGroup.ImageAlternateTextFormat"), promotionGroup.Name)
            };

            return pictureModel;
        });
        var seName = _urlRecordService.GetSeName(promotionGroup);
        return new PromotionMegaMenuModel
        {
            FullSizeImageUrl = bannerPictureModel.FullSizeImageUrl,
            SeName = seName
        };
    }
    ...
    /// </summary>
    /// <param name="promotionGroup">PromotionGroup identifier</param>
    /// <param name="command">Catalog paging filtering command</param>
    /// <returns>PromotionGroup model</returns>
    public override PromotionGroupModel PreparePromotionGroupModel(PromotionGroup promotionGroup, CatalogPagingFilteringModel command)
    {
        if (promotionGroup == null)
            throw new ArgumentNullException(nameof(promotionGroup));

        var model = new PromotionGroupModel
        {
            Id = promotionGroup.Id,
            Name = _hasMultipleLanguages ? _localizationService.GetLocalized(promotionGroup, x => x.Name) : promotionGroup.Name,
            Description = _hasMultipleLanguages ? _localizationService.GetLocalized(promotionGroup, x => x.Description) : promotionGroup.Description,
            MetaKeywords = _hasMultipleLanguages ? _localizationService.GetLocalized(promotionGroup, x => x.MetaKeywords) : promotionGroup.MetaKeywords,
            MetaDescription = _hasMultipleLanguages ? _localizationService.GetLocalized(promotionGroup, x => x.MetaDescription) : promotionGroup.MetaDescription,
            MetaTitle = _hasMultipleLanguages ? _localizationService.GetLocalized(promotionGroup, x => x.MetaTitle) : promotionGroup.MetaTitle,
            SeName = _urlRecordService.GetSeName(promotionGroup),
        };
        ...
        //page size
        PreparePageSizeOptions(model.PagingFilteringContext, command,
            promotionGroup.AllowCustomersToSelectPageSize,
            promotionGroup.PageSizeOptions,
            promotionGroup.PageSize);

        //price ranges
        model.PagingFilteringContext.PriceRangeFilter.LoadPriceRangeFilters(promotionGroup.PriceRanges, _webHelper, _priceFormatter);
        var selectedPriceRange = model.PagingFilteringContext.PriceRangeFilter.GetSelectedPriceRange(_webHelper, promotionGroup.PriceRanges);
        decimal? minPriceConverted = null;
        decimal? maxPriceConverted = null;
        if (selectedPriceRange != null)
        {
        ...
        if (!_catalogSettings.IgnoreFeaturedProducts)
        {
            ...
            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.PromotionGroupHasFeaturedProductsKey,
                promotionGroup,
            ...
            {
                //cache indicates that the promotionGroup has featured products
                //let's load them
                featuredProducts = _productService.SearchProducts(
                   promotionGroup.Id,
                   isInHolding: false,
        ...
        //prepare picture model
        var pictureSize = _mediaSettings.PromotionGroupThumbPictureSize;
        var promotionGroupPictureCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.PromotionGroupPictureModelKey,
            promotionGroup, pictureSize, true, _workContext.WorkingLanguage,
            _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore);
        model.PictureModel = _staticCacheManager.Get(promotionGroupPictureCacheKey, () =>
        {
            ...
            var pictureModel = new PictureModel
            {
                FullSizeImageUrl = _pictureService.GetPictureUrl(ref picture),
                ImageUrl = _pictureService.GetPictureUrl(ref picture, pictureSize),
                Title = string.Format(_localizationService.GetResource("Media.PromotionGroup.ImageLinkTitleFormat"), model.Name),
                AlternateText = string.Format(_localizationService.GetResource("Media.PromotionGroup.ImageAlternateTextFormat"), model.Name)
            };
        ...
        //prepare banner picture model
        pictureSize = _mediaSettings.PromotionGroupThumbPictureSize;
        var promotionGroupBannerPictureCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.PromotionGroupBannerPictureModelKey,
            promotionGroup, pictureSize, true, _workContext.WorkingLanguage,
            _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore);
        model.BannerPictureModel = _staticCacheManager.Get(promotionGroupBannerPictureCacheKey, () =>
        {
            var picture = _pictureService.GetPictureById(promotionGroup.BannerPictureId);
            var pictureModel = new PictureModel
            {
        ...
        //prepare discounts
        var discounts = _promotionGroupService.GetDiscountsForPromotionGroup(model.Id);
        foreach (var discount in discounts)
        {
            model.Discounts.Add(_discountFactory.PrepareDiscountModel(null, discount));
        }
    ...
    /// <summary>
    /// Prepare promotionGroup template view path
    /// </summary>
    /// <param name="templateId">Template identifier</param>
    /// <returns>PromotionGroup template view path</returns>
    public override string PreparePromotionGroupTemplateViewPath(int templateId)
    {
        var template = _promotionGroupTemplateService.GetPromotionGroupTemplateById(templateId) ??
                       _promotionGroupTemplateService.GetAllPromotionGroupTemplates(storeId: _storeContext.ActiveStoreScopeConfiguration).FirstOrDefault();
    ...
    /// <summary>
    /// Prepare promotionGroup all models
    /// </summary>
    /// <returns>List of promotionGroup models</returns>
    public override List<PromotionGroupModel> PreparePromotionGroupAllModels()
    {
        var model = new List<PromotionGroupModel>();
        var promotionGroups = _promotionGroupService.GetAllPromotionGroups(storeId: _storeContext.ActiveStoreScopeConfiguration);
        foreach (var promotionGroup in promotionGroups)
        {
            var modelMan = new PromotionGroupModel
            {
                Id = promotionGroup.Id,
                Name = _hasMultipleLanguages ? _localizationService.GetLocalized(promotionGroup, x => x.Name) : promotionGroup.Name,
                Description = _hasMultipleLanguages ? _localizationService.GetLocalized(promotionGroup, x => x.Description) : promotionGroup.Description,
            ...
            var promotionGroupPictureCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.PromotionGroupPictureModelKey,
                promotionGroup, pictureSize, true, _workContext.WorkingLanguage,
                _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore);
            modelMan.PictureModel = _staticCacheManager.Get(promotionGroupPictureCacheKey, () =>
            {
                var picture = _pictureService.GetPictureById(promotionGroup.PictureId);
                var pictureModel = new PictureModel
                {
                    FullSizeImageUrl = _pictureService.GetPictureUrl(ref picture),
                    ImageUrl = _pictureService.GetPictureUrl(ref picture, pictureSize),
                    Title = string.Format(_localizationService.GetResource("Media.PromotionGroup.ImageLinkTitleFormat"), modelMan.Name),
                    AlternateText = string.Format(_localizationService.GetResource("Media.PromotionGroup.ImageAlternateTextFormat"), modelMan.Name)
                };
            ...
            //prepare discounts
            var discounts = _promotionGroupService.GetDiscountsForPromotionGroup(modelMan.Id);
            foreach (var discount in discounts)
            {
                modelMan.Discounts.Add(_discountFactory.PrepareDiscountModel(null, discount));
            }
    ...
    public override BestSellersModel GetBestSellers(int storeId, CatalogPagingFilteringModel command)
    {
        var bestSellers = _orderReportService.BestSellersReport(storeId: storeId);

        var productIds = bestSellers.Select(i => i.ProductId).ToArray();

        var products = _productService.GetProductsByIds(productIds);

        var model = new BestSellersModel();

        var query = _productModelFactory.PrepareProductOverviewModels(products);
        ...
        model.PagingFilteringContext.OrderBy = command.OrderBy ?? (int?)ProductSortingEnum.Position;

        switch ((ProductSortingEnum)model.PagingFilteringContext.OrderBy)
        {
            ...
            case ProductSortingEnum.PriceAsc:
                query = query.OrderBy(i => i.ProductPrice.PriceValue);
                break;
            case ProductSortingEnum.PriceDesc:
                query = query.OrderByDescending(i => i.ProductPrice.PriceValue);
                break;
        }
