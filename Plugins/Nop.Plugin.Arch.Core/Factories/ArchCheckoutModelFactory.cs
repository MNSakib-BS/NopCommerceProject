...
public partial class ArchCheckoutModelFactory : ICheckoutModelFactory
{
    #region Fields
    ...
    public ArchCheckoutModelFactory(AddressSettings addressSettings,
    ...
    public virtual GetShippingDistance CanCalculateDeliveryRange(int addressId)
    {
        var activeShippingPlugins = _shippingPluginManager.LoadActivePlugins();
        var shippingDistanceModel = new GetShippingDistance();

        foreach (var plugin in activeShippingPlugins)
        {
            if (plugin is IGetDistanceFromStore getDistanceFromStorePlugin)
            {
                shippingDistanceModel = getDistanceFromStorePlugin.IsFarFromStore(_storeContext.CurrentStore, addressId);
            }
        }

        return shippingDistanceModel;
    }

    public virtual bool? CanCalculateDeliveryFeeRate(int addressId)
    {
        var activeShippingPlugins = _shippingPluginManager.LoadActivePlugins();

        foreach (var plugin in activeShippingPlugins)
        {
            if (plugin is IGetRate getRatePlugin)
            {
                var rate = getRatePlugin.GetRate(_storeContext.CurrentStore.Id, addressId, new ShippingMethod
                {
                    Name = ArchShippingMethod.Delivery.ToString()
                });
                ...
                if (customer != null)
                {
                    isDebtor = customer.IsDebtor(_storeContext.CurrentStore.Id);
                }

                if (rate.Value == 0 && !isDebtor)
                    return false;
            }
        }
    ...
    public virtual async Task<CheckoutPaymentMethodModel> PreparePaymentMethodModelAsync(IList<ShoppingCartItem> cart, int filterByCountryId)
    {
        ...
        // debtor open to buy
        var debtorOpenToByAttributeKey = _cacheKeyService.PrepareKeyForShortTermCache(NopOrderDefaults.DebtorOpenToByAttributeCacheKey, _workContext.CurrentCustomer, _storeContext.CurrentStore.Id);
        var openToBuy = _genericAttributeService.GetAttribute(_workContext.CurrentCustomer, debtorOpenToByAttributeKey.Key, _storeContext.CurrentStore.Id, default(decimal));

        var debtorPaidOnAccountReserveAttributeKey = _cacheKeyService.PrepareKeyForShortTermCache(NopOrderDefaults.DebtorPaidOnAccountReserveAttributeCacheKey, _workContext.CurrentCustomer, _storeContext.CurrentStore.Id);
        var paidOnAccount = _genericAttributeService.GetAttribute(_workContext.CurrentCustomer, debtorPaidOnAccountReserveAttributeKey.Key, _storeContext.CurrentStore.Id, default(decimal));
        var debtorNumber = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer, NopCustomerDefaults.DebtorNumberAttribute, _storeContext.CurrentStore.Id);

        var isDebtor = _workContext.CurrentCustomer.IsDebtor(_storeContext.CurrentStore.Id);
        var debtorType = _workContext.CurrentCustomer.DebtorType(_storeContext.CurrentStore.Id);
        var debtorStatus = _workContext.CurrentCustomer.DebtorAccountStatus(_storeContext.CurrentStore.Id);

        var hasValidAccountStatus = debtorStatus == DebtorAccountStatus.Good ||
                                    debtorStatus == DebtorAccountStatus.Fair ||
                                    debtorStatus == DebtorAccountStatus.Bad;

        model.DebtorAccountBox.Display = isDebtor && debtorType == DebtorType.Credit && hasValidAccountStatus && !String.IsNullOrEmpty(debtorNumber);
        model.DebtorAccountBox.OpenToBuy = _priceFormatter.FormatPrice(openToBuy, true, false);
        model.DebtorAccountBox.IsApplied = paidOnAccount > 0 && paidOnAccount <= openToBuy;
        model.DebtorAccountBox.PaidOnAccount = paidOnAccount;

        var openToBuyFormatted = _priceFormatter.FormatPrice(openToBuy, true, false);
        var paidOnAccountFormatted = _priceFormatter.FormatPrice(paidOnAccount, true, false);

        model.DebtorAccountBox.Message = model.DebtorAccountBox.IsApplied
            ? string.Format(_localizationService.GetResource("ShoppingCart.DebtorAccountBox.Applied"), paidOnAccountFormatted, openToBuyFormatted)
            : paidOnAccount == 0
                ? string.Format(_localizationService.GetResource("ShoppingCart.DebtorAccountBox.Applied"), paidOnAccountFormatted, openToBuyFormatted)
                : string.Format(_localizationService.GetResource("ShoppingCart.DebtorAccountBox.InvalidAmount"), paidOnAccountFormatted, openToBuyFormatted);
