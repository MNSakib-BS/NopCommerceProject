...
public partial class CommonController : BasePublicController
{
    ...
    [HttpPost, ActionName("ContactUltra")]
    [FormValueRequired("send-advanced-email")]
    [ValidateCaptcha]
    //available even when a store is closed
    [CheckAccessClosedStore(true)]
    public virtual IActionResult UltraContactUsSend(UltraContactUsModel model, bool captchaValid)
    {
        //validate CAPTCHA
        if (_captchaSettings.Enabled && _captchaSettings.ShowOnContactUsPage && !captchaValid)
        {
        ...
        if (_commonSettings.DisplayContactUsAttachment && _commonSettings.ContactUsAttachmentFileSize != 0 && httpPostedFile != null)
        {
            var attachmentMaxFileSize = _commonSettings.ContactUsAttachmentFileSize;
            if (httpPostedFile.Length > (attachmentMaxFileSize * 1024))
            {
                var errorMessage = _localizationService.GetResource(("ContactUs.Attachment.MaximumUploadedFileSize"));
                ModelState.AddModelError("", string.Format(errorMessage, attachmentMaxFileSize / 1024));
            }
        }
        model = _commonModelFactory.PrepareUltraContactUsModel(model, true);

        if (ModelState.IsValid)
        {
            var displayAdvanced = true; //Because we're in this method we can assume this is true
            var contactUsModel = model.ContactUsModel;
            string subject = null;
            if (model.AreaOfConcernOptions.Any())
            {
                subject = _storeContext.CurrentStore.Name + " " + _localizationService.GetLocalizedEnum((AreaOfConcern)model.AreaOfConcernId);
            }
            var body = Nop.Core.Html.HtmlHelper.FormatText(contactUsModel.Enquiry, false, true, false, false, false, false);
            var province = model.AvailableProvince.Where(i => i.Value == model.ProvinceId.ToString()).FirstOrDefault();
            var provinceName = province == null ? string.Empty : province.Text;
            var enquiryType = subject ?? "Contact Us Form";

            var advancedTokens = new List<Token>
                {
                    new Token("ContactUs.DisplayAdvanced", displayAdvanced),
                    new Token("ContactUs.ContactNumber", model.ContactNumber),
                    new Token("ContactUs.IsSiteIdentified", model.IsSiteIdentified),
                    new Token("ContactUs.HasLiquorLicense", model.HasLiquorLicense),
                    new Token("ContactUs.Province", provinceName),
                    new Token("ContactUs.Suburb", model.Suburb),
                    new Token("ContactUs.LiquorStoreEnquiry", enquiryType)
                };

            if (httpPostedFile != null)
            {
                var fileInfo = FileUploader(httpPostedFile);
                _workflowMessageService.SendContactUsMessage(_workContext.WorkingLanguage.Id,
                    contactUsModel.Email.Trim(), contactUsModel.FullName, subject, body, fileInfo.FullName, fileInfo.Name, displayAdvanced, advancedTokens);
            }
            else
            {
                _workflowMessageService.SendContactUsMessage(_workContext.WorkingLanguage.Id,
                    contactUsModel.Email.Trim(), contactUsModel.FullName, subject, body, advancedContactUs: displayAdvanced, advancedTokens: advancedTokens);
            }


            contactUsModel.Result = _localizationService.GetResource("ContactUs.YourEnquiryHasBeenSent");
            ...
            _notificationService.SuccessNotification(contactUsModel.Result);
            return RedirectToAction(nameof(ContactUltra), new { areaOfConcernId = model.AreaOfConcernId });
        }
        model.ContactUsModel = _commonModelFactory.PrepareContactUsModel(model.ContactUsModel, true);
        return View(model);
    }

    private FileInfo FileUploader(IFormFile httpPostedFile)
    {
        var contactUsAttachmentDirectory = _fileProvider.MapPath("~/wwwroot/contactus");
        if (!_fileProvider.DirectoryExists(contactUsAttachmentDirectory))
            _fileProvider.CreateDirectory(contactUsAttachmentDirectory);

        if (httpPostedFile.Length > 0)
        {
            using (var ms = new MemoryStream())
            {
                var filePath = $"{contactUsAttachmentDirectory}\\{httpPostedFile.FileName}";
    ...
    /// <summary>
    /// Replace tokens from a template file and create a new file with values from tokens.
    /// </summary>
    /// <remarks>
    /// Requires a templatePath, filePath and a dictionary of tokenValues.
    /// 
    /// Example of expected values for tokenValues.
    /// var tokenValues = new Array();
    /// tokenValues[0] = { key: 'token1', value: 'value1' };
    /// tokenValues[1] = { key: 'token2', value: 'value2' };
    /// </remarks>
    [HttpPost]
    [IgnoreAntiforgeryToken]
    public IActionResult ReplaceTemplateTokens()
    {
        // get request data
        ...
        if (string.IsNullOrWhiteSpace(templatePath) ||
            ...
            !fileProvider.FileExists(templatePath))
            return NoContent();

        // get template content
        var text = fileProvider.ReadAllText(templatePath, Encoding.UTF8);

        // replace tokens
        foreach (var tokenValue in tokenValues)
        {
            var key = $"[#{tokenValue["key"].Value<string>()}#]"; // token format => [#key#]
            var value = tokenValue["value"].Value<string>();

            text = text.Replace(key, value);
        }

        // save replaced content
