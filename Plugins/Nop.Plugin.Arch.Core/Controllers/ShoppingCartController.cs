...
public partial class ShoppingCartController : BasePublicController
{
    ...
    public virtual IActionResult AddProductToWishlist_Details(int productId, int shoppingCartTypeId, int wishlistGroupId, IFormCollection form, int quantity = 1)
    {
        ...
        if (wishlistGroupId < 0)
        {
            var newWishlistQuantity = (form.Count <= 0) ? quantity : _productAttributeParser.ParseEnteredQuantity(product, form);
            //redirect to the new wishlist page
            return Json(new
            {
                redirect = $"newwishlist/{productId}/{newWishlistQuantity}"
            });
        }
        ...
        if (_shoppingCartSettings.AllowCartItemEditing && updatecartitemid > 0)
        {
            ...
            updatecartitem = cart.FirstOrDefault(x => x.Id == updatecartitemid && x.WishListGroupId == wishlistGroupId);
        ...
        //entered quantity
        var enteredQuantity = (form.Count <= 0) ? quantity : _productAttributeParser.ParseEnteredQuantity(product, form);
        ...
        SaveItem(updatecartitem, addToCartWarnings, product, cartType, attributes, customerEnteredPriceConverted, rentalStartDate, rentalEndDate, enteredQuantity, wishlistGroupId);
    ...
    [HttpPost]
    [IgnoreAntiforgeryToken]
    public virtual IActionResult AddProductToCart_Default(int shoppingCartTypeId, IFormCollection form)
    {
        var builder = new StringBuilder();
        int errorCount = 0;

        var wishlistGroup = form["wishlistgroup"];
        var productIds = form["products"];
        ...
        foreach (var productIdString in productIds)
        {
            ...
            var mappings = _productAttributeService.GetProductAttributeMappingsByProductId(productId);
            ...
            //update existing shopping cart item
            var updatecartitemid = 0;
            foreach (var formKey in productForm.Keys)
                if (formKey.Equals($"addtocart_{productId}.UpdatedShoppingCartItemId", StringComparison.InvariantCultureIgnoreCase))
                {
                    int.TryParse(productForm[formKey], out updatecartitemid);
                    break;
                }

            var wishListGroupidFound = 0;
            int.TryParse(wishlistGroup.FirstOrDefault(), out wishListGroupidFound);
            int? wishListGroupid = wishListGroupidFound == 0 ? (int?)null : wishListGroupidFound;
            ...
            //customer entered price
            var customerEnteredPriceConverted = _productAttributeParser.ParseCustomerEnteredPrice(product, productForm);

            //entered quantity
            var quantity = _productAttributeParser.ParseEnteredQuantity(product, productForm);

            //product and gift card attributes
            var attributes = _productAttributeParser.ParseProductAttributes(product, productForm, addToCartWarnings);

            //rental attributes
            _productAttributeParser.ParseRentalDates(product, productForm, out var rentalStartDate, out var rentalEndDate);

            var cartType = updatecartitem == null ? (ShoppingCartType)shoppingCartTypeId :
                //if the item to update is found, then we ignore the specified "shoppingCartTypeId" parameter
                updatecartitem.ShoppingCartType;

            SaveItem(updatecartitem, addToCartWarnings, product, cartType, attributes, customerEnteredPriceConverted, rentalStartDate, rentalEndDate, quantity, wishListGroupid);

            //return result
            if (addToCartWarnings.Any())
            {
                builder.Append($"Unable to add {product.Name}.<br>");
                errorCount++;
            }
        }
            ...
            case ShoppingCartType.Wishlist:
                {
                    ...
                    builder.Append(string.Format(_localizationService.GetResource("Products.ProductsHaveBeenAddedToTheWishlist.Link"), productIds.Count - errorCount, Url.RouteUrl("Wishlist")));
            ...
            default:
                {
                    ...
                    var miniModel = _shoppingCartModelFactory.PrepareMiniShoppingCartModel();
                    var updatetopcartsubtotal = miniModel.SubTotal;

                    builder.Append(string.Format(_localizationService.GetResource("Products.ProductsHaveBeenAddedToTheCart.Link"), productIds.Count - errorCount, Url.RouteUrl("ShoppingCart")));
    ...
    [HttpPost, ActionName("Cart")]
    [FormValueRequired("applyloyaltycardnumber")]
    public virtual IActionResult ApplyLoyaltyCardNumber(string loyaltycardnumber, IFormCollection form)
    {
        //trim
        ...
        if (!string.IsNullOrWhiteSpace(loyaltycardnumber))
        {
            ...
            customer.LoyaltyCardNumber = loyaltycardnumber;
            _customerService.UpdateCustomer(customer);

            model.LoyaltyCardBox.Message = _localizationService.GetResource("ShoppingCart.LoyaltyCardNumber.Applied");
            model.LoyaltyCardBox.IsApplied = true;
        }
        else
        {
            model.LoyaltyCardBox.Message = _localizationService.GetResource("ShoppingCart.LoyaltyCardNumber.Empty");
            model.LoyaltyCardBox.IsApplied = false;
        }
    ...
    public virtual IActionResult ClearShoppingCart()
    {
        ...
        //Discard the order with the specified tracking number
        var transactionTrackingNumber = cart.FirstOrDefault().OrderTrackingNumber;
        if (transactionTrackingNumber.HasValue && transactionTrackingNumber != 0M)
            _archApi.DiscardOrder((decimal)transactionTrackingNumber, out var _);

        //Cleanup the transaction tracking number value
        var transactionTrackingNumberAttributeCacheKey = _cacheKeyService.PrepareKeyForShortTermCache(NopOrderDefaults.TransactionTrackingNumberAttributeCacheKey, _workContext.CurrentCustomer, _storeContext.CurrentStore.Id);
        _genericAttributeService.DeleteAttributes(_workContext.CurrentCustomer, transactionTrackingNumberAttributeCacheKey.Key, _storeContext.CurrentStore.Id);
        // Force clear the Cart Cache to ensure we load the cart correcly
        _staticCacheManager.RemoveByPrefix(NopOrderDefaults.ShoppingCartPrefixCacheKey);
        _shoppingCartService.UpdateCartReminder();
    ...
    [HttpPost]
    [IgnoreAntiforgeryToken]
    public virtual IActionResult AddProductToWishlist(IFormCollection form, int productId)
    {
        var product = _productService.GetProductById(productId);
        if (product == null)
            return Json(new { redirect = Url.RouteUrl("Homepage") });

        if (product.ProductType != ProductType.SimpleProduct)
            return Json(new { success = false, message = "Only simple products could be added to the cart" });

        string updateCartItemIdString = form[$"addtocart_{productId}.UpdatedShoppingCartItemId"];
        int.TryParse(updateCartItemIdString, out var updateCartItemId);

        string[] wishlistGroups = form["wishlist-groups"];

        if (wishlistGroups.Length == 0)
            return Json(new { success = false, message = "No wishlist selected" });

        var addToCartWarnings = new List<string>();

        foreach (var wishlistGroupIdStrings in wishlistGroups)
        {
            var isValid = int.TryParse(wishlistGroupIdStrings, out var wishlistGroupId);
            if (isValid)
            {
                if (wishlistGroupId == -1)
                {
                    string wishlistName = form["wishlist-name"];

                    if (string.IsNullOrEmpty(wishlistName))
                        return Json(new { success = false, message = "No wishlist name provided" });

                    var newWishlistGroup = new WishListGroup()
                    {
                        Name = wishlistName,
                    ...
                    _wishListService.AddWishList(newWishlistGroup);

                    wishlistGroupId = newWishlistGroup.Id;
                }
                ...
                if (_shoppingCartSettings.AllowCartItemEditing && updateCartItemId > 0)
                {
                    ...
                    if (updatecartitem != null && product.Id != updatecartitem.ProductId)
                        return Json(new { success = false, message = "This product does not match a passed shopping cart item identifier" });
                }
                ...
                SaveItem(updatecartitem, addToCartWarnings, product, ShoppingCartType.Wishlist, attributes, customerEnteredPriceConverted, rentalStartDate, rentalEndDate, quantity, wishlistGroupId);
            }
        }

        //return result
        return GetProductToCartDetails(addToCartWarnings, ShoppingCartType.Wishlist, product);
    }
    ...
    [HttpsRequirement]
    public virtual IActionResult CreateWishList(int productId, int quantity = 1)
    {
        if (!_customerService.IsRegistered(_workContext.CurrentCustomer))
            return Challenge();

        return View(new CreateWishListModel() { ProductIds = new int[] { productId }, Quantity = quantity });
    }

    [HttpPost, ActionName("Cart")]
    [FormValueRequired("addtolist")]
    public virtual IActionResult CreateWishList(IFormCollection form)
    {
        var shoppingCartItemIds = (from id in form["shoppingCartItemIds"]
                                   select Convert.ToInt32(id)).ToArray();

        return View("~/Views/ShoppingCart/CreateWishList.cshtml", new CreateWishListModel() { ShoppingCartItemIds = shoppingCartItemIds });
    }

    [HttpsRequirement]
    [HttpPost]
    public virtual IActionResult CreateWishList(CreateWishListModel createWishListModel)
    {
        if (ModelState.IsValid)
        {
            var wishListGroup = new WishListGroup()
            {
                Name = createWishListModel.Name,
                CustomerId = _workContext.CurrentCustomer.Id,
                StoreId = _storeContext.CurrentStore.Id
            };
            _wishListService.AddWishList(wishListGroup);

            if (createWishListModel.ShoppingCartItemIds?.Length > 0)
            {
                var cart = _shoppingCartService.GetShoppingCart(_workContext.CurrentCustomer, ShoppingCartType.ShoppingCart);
                foreach (var id in createWishListModel.ShoppingCartItemIds)
                {
                    var cartItem = cart.FirstOrDefault(i => i.Id == id);
                    if (cartItem != null)
                    {
                        var product = _productService.GetProductById(cartItem.ProductId);
                        _shoppingCartService.AddToCart(_workContext.CurrentCustomer, product, ShoppingCartType.Wishlist, wishListGroup.Id, _storeContext.ActiveStoreScopeConfiguration,
                                                       cartItem.AttributesXml, cartItem.CustomerEnteredPrice, cartItem.RentalStartDateUtc,
                                                       cartItem.RentalEndDateUtc, cartItem.Quantity);
                    }
                }
            }

            if (createWishListModel.ProductIds?.Length > 0)
            {
                var dictionary = new Dictionary<string, StringValues>();
                dictionary.Add("wishlistgroup", wishListGroup.Id.ToString());
                dictionary.Add("products", string.Join(',', createWishListModel.ProductIds));
                dictionary.Add("quantity", string.Join(',', createWishListModel.Quantity));

                AddProductToCart_Default((int)ShoppingCartType.Wishlist, new FormCollection(dictionary));
            }

            return RedirectToRoute("Wishlist");
        }

        return View(createWishListModel);
    }

    public virtual IActionResult CreateNewWishList(string Name, int[] ShoppingCartItemIds)
    {
        var wishListGroup = new WishListGroup()
        {
            Name = Name,
    ...
    [HttpPost, ActionName("Wishlist")]
    [FormValueRequired("savewishlistbutton")]
    public virtual IActionResult SaveWishlist(Guid? customerGuid, IFormCollection form)
    {
        ...
        int? wishlistGroupId = form.ContainsKey("wishlistGroupId")
            ? form["wishlistGroupId"].ToString().Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(int.Parse)
                .FirstOrDefault()
            : (int?)null;

        var pageCart = _shoppingCartService.GetShoppingCart(pageCustomer, wishlistGroupId, shoppingCartType: ShoppingCartType.Wishlist, storeId: _storeContext.CurrentStore.Id);
        ...
        var wishlistGroup = _wishListService.GetWishListGroups(pageCustomer.Id, _storeContext.CurrentStore.Id).Where(x => x.Id == wishlistGroupId).FirstOrDefault();
        ...
        foreach (var sci in pageCart)
        {
            if (allIdsToAdd.Contains(sci.Id))
            {
                ...
                var customerEnteredPriceConverted = _productAttributeParser.ParseCustomerEnteredPrice(product, form);
                var attributes = _productAttributeParser.ParseProductAttributes(product, form, allWarnings);
                _productAttributeParser.ParseRentalDates(product, form, out var rentalStartDate, out var rentalEndDate);
                var quantity = sci.Quantity;
                SaveItem(null, allWarnings, product, ShoppingCartType.Wishlist, attributes, customerEnteredPriceConverted, rentalStartDate, rentalEndDate, quantity, wishlistGroupId);
            }
        }
        ...
        var cart = _shoppingCartService.GetShoppingCart(pageCustomer, wishlistGroupId, shoppingCartType: ShoppingCartType.Wishlist, storeId: _storeContext.CurrentStore.Id);

        var model = new WishlistModel();
        model = _shoppingCartModelFactory.PrepareWishlistModel(model, cart, new WishlistModelPagingFilteringModel(), !customerGuid.HasValue);
        model.SelectedWishListGroupId = wishlistGroupId;

        _notificationService.SuccessNotification(string.Format(_localizationService.GetResource("Wishlist.WishlistSaved"), Url.RouteUrl("Wishlist")));
    ...
    [HttpPost, ActionName("Wishlist")]
    [FormValueRequired("deletewishlist")]
    public virtual IActionResult DeleteWishlist(Guid? customerGuid, IFormCollection form)
    {
        ...
        var pageCart = _shoppingCartService.GetShoppingCart(pageCustomer, wishlistGroupId, false, ShoppingCartType.Wishlist, _storeContext.CurrentStore.Id);
        ...
        _wishListService.DeleteWishListGroup(wishlistGroupId.GetValueOrDefault());

        var cart = _shoppingCartService.GetShoppingCart(pageCustomer, null, false, ShoppingCartType.Wishlist, _storeContext.CurrentStore.Id);
    ...
    [HttpPost]
    [IgnoreAntiforgeryToken]
    public virtual IActionResult MoveToWishlist(int wishlistId, int shoppingCartItemId)
    {
        var shoppingCartItems = _shoppingCartService.GetShoppingCart(_workContext.CurrentCustomer, ShoppingCartType.Wishlist);

        var shoppingCartItem = shoppingCartItems.FirstOrDefault(i => i.Id == shoppingCartItemId);
        ...
        var alreadyExists = shoppingCartItems.Any(i => i.ProductId == shoppingCartItem.ProductId && i.WishListGroupId == wishlistId);
        if (alreadyExists)
        {
            return Json(new
            {
        ...
        shoppingCartItem.WishListGroupId = wishlistId == 0 ? null : (int?)wishlistId;
    ...
    [HttpPost]
    [IgnoreAntiforgeryToken]
    public virtual IActionResult UpdateWishlistName(int wishlistId, string name)
    {
        var wishlistGroup = _wishListService.GetWishListGroup(wishlistId);

        wishlistGroup.Name = name;

        _wishListService.UpdateWishListName(wishlistGroup);
