...
public partial class CheckoutController : BasePublicController
{
    ...
    protected virtual void SetDistanceFromStoreToShippingAddress(Store store, Order order)
    {


        var distanceFromStoreToShippingAddress = string.Empty;

        var address = _addressService.GetAddressById(order.ShippingAddressId.HasValue
            ? order.ShippingAddressId.Value
            : order.BillingAddressId);

        var addressModel = new Nop.Web.Models.Common.AddressModel();
        _addressModelFactory.PrepareAddressModel(addressModel,
            address: address,
            excludeProperties: false,
            addressSettings: _addressSettings);

        _googleApi.Initialize();
        var distanceMatrix = _googleApi.GetDistanceMatrixAsync(store, addressModel.ToString()).GetAwaiter().GetResult();
        distanceFromStoreToShippingAddress = distanceMatrix?.Rows?.FirstOrDefault()?.Elements?.FirstOrDefault()?.Distance?.Text;

        var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopOrderDefaults.DistanceFromStoreToShippingAddressKey, order.Id).Key;
        _genericAttributeService.SaveAttribute(order, cacheKey, distanceFromStoreToShippingAddress, order.StoreId);

        //NOTE : DEBUG LOG FOR CHECKOUT
        var customerDebug = _workContext.CurrentCustomer;
        _logger.InsertLog(LogLevel.Debug, $"SetDistanceFromStoreToShippingAddress After Checks., orderId = {order.Id}, distanceFromStoreToShippingAddress = {distanceFromStoreToShippingAddress}...(ifEmptyitsBrokenOnDistanceMatrix - Checkoutcontroller.170)", customer: customerDebug);
    ...
    public virtual IActionResult UpdateShippingMethod(int addressId)
    {
        //validation
        if (_orderSettings.CheckoutDisabled)
            return RedirectToRoute("ShoppingCart");
        ...
        var model = _checkoutModelFactory.UpdateShippingMethodWithNewAddress(cart, address);

        return Json(new
        {
            Model = model.CheckoutShippingMethodModel,
            Address = $"{address.Address1}," + $"{address.City}",
            DeliveryFee = model.CheckoutShippingMethodModel.ShippingMethods.FirstOrDefault().Fee,
            AddressId = addressId,
        });
    }
    ...
    [HttpPost]
    [IgnoreAntiforgeryToken]
    public virtual async Task<IActionResult> InputNewShippingAddressAsync(CheckoutShippingAddressModel model, IFormCollection form)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            if (address == null)
            {
                ...
                var validAddress = await _googleApi.IsValidAddress(newAddress.ToString());

                if (!validAddress)
                {
                    return Json(new
                    {
                        Error = "Sorry we couldn't find that address, Please ensure all your details are correct and try again",
                    });
                }
            ...
            var outofRangeError = _localizationService.GetResource("Address.Error.OutofRange");
            var canCalculateDeliveryRange = _checkoutModelFactory.CanCalculateDeliveryRange(address.Id);
            if (!canCalculateDeliveryRange.IsFarFromStore.HasValue || canCalculateDeliveryRange.IsFarFromStore.Value)
            {
                ModelState.AddModelError("", outofRangeError);
            ...
            {
                var canCalculateDeliveryFeeRate = _checkoutModelFactory.CanCalculateDeliveryFeeRate(address.Id);
                if (!canCalculateDeliveryFeeRate.HasValue || !canCalculateDeliveryFeeRate.Value)
                {
                    ModelState.AddModelError("", outofRangeError);
                    return Json(new
                    {
            ...
            return UpdateShippingMethod(address.Id);
    ...
    [HttpPost]
    [IgnoreAntiforgeryToken]
    public virtual IActionResult DisplayShippingAddress()
    {
        var cart = _shoppingCartService.GetShoppingCart(_workContext.CurrentCustomer, ShoppingCartType.ShoppingCart, _storeContext.CurrentStore.Id);
