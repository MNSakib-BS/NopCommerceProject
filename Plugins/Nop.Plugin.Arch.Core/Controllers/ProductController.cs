...
public partial class ProductController : BasePublicController
{
    ...
    public virtual IActionResult NewProductsRss()
    {
        var feed = new RssFeed(
            $"{(_hasMultipleLanguages ? _localizationService.GetLocalized(_storeContext.CurrentStore, x => x.Name) : _storeContext.CurrentStore.Name)}: New products",
        ...
        foreach (var product in products)
        {
            ...
            var productName = _hasMultipleLanguages ? _localizationService.GetLocalized(product, x => x.Name) : product.Name;
            var productDescription = _hasMultipleLanguages ? _localizationService.GetLocalized(product, x => x.ShortDescription) : product.ShortDescription;
    ...
    #endregion
    public virtual IActionResult GetProductBrandFromSlug(string productSlug)
    {
        var urlRecord = _urlRecordService.GetBySlug(productSlug.Replace("/", ""));
        ...
        return new JsonResult(product.BrandNameField);
    }
}