...
public partial class ArchSettingController : BaseAdminController
{
    #region Fields
    ...
    public ArchSettingController(IProductService productService,
    ...
    #region Utilities
    private void ProcessCatalogSettingsForProducts(IPagedList<Product> products)
    {
    ...
    private async void ProcessArchApiSettingsForProducts(IPagedList<Product> products,
        ArchApiSettings archApiSettings,
        int oldOrderMaximumQuantityDefault)
    {
        foreach (var product in products)
        {
            var origManageInventoryMethod = product.ManageInventoryMethod;
            var origDisplayStockAvailability = product.DisplayStockAvailability;
            var origDisplayStockQuantity = product.DisplayStockQuantity;
            var origOrderMaximumQuantity = product.OrderMaximumQuantity;

            product.ManageInventoryMethod = archApiSettings.TrackInventory ? ManageInventoryMethod.ManageStock : ManageInventoryMethod.DontManageStock;
            product.DisplayStockAvailability = archApiSettings.DisplayStockAvailability;
            product.DisplayStockQuantity = archApiSettings.DisplayStockQuantity;

            // only update the quantity if the original value is equal to the old default setting
            // this ensures that the value can be overridden from the portal for values that are not equal to the default setting
            product.OrderMaximumQuantity = oldOrderMaximumQuantityDefault == origOrderMaximumQuantity
                ? archApiSettings.OrderMaximumQuantityDefault : product.OrderMaximumQuantity;

            var hasChanged = origManageInventoryMethod != product.ManageInventoryMethod ||
                             origDisplayStockAvailability != product.DisplayStockAvailability ||
                             origDisplayStockQuantity != product.DisplayStockQuantity ||
                             origOrderMaximumQuantity != product.OrderMaximumQuantity;
            if (hasChanged)
