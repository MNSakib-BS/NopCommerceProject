...
{
    public partial class ArchCategoryController : BaseAdminController
    {
        #region Fields
        ...
        public ArchCategoryController(IAclService aclService,
        ...
        protected virtual async Task UpdatePictureSeoNamesAsync(Category category)
        {
            ...
            var bannerHeroPictureTop = _pictureService.GetPictureByIdAsync(category.BannerHeroPictureTopId);
            var bannerHeroPictureLower = _pictureService.GetPictureByIdAsync(category.BannerHeroPictureLowerId);

            if (picture != null)
                await _pictureService.SetSeoFilenameAsync(picture.Id, await _pictureService.GetPictureSeNameAsync(category.Name));

            if (bannerHeroPictureTop != null)
                await _pictureService.SetSeoFilenameAsync(bannerHeroPictureTop.Id, await _pictureService.GetPictureSeNameAsync(category.Name));

            if (bannerHeroPictureLower != null)
                await _pictureService.SetSeoFilenameAsync(bannerHeroPictureLower.Id, await _pictureService.GetPictureSeNameAsync(category.Name));
        ...
        public virtual async Task<IActionResult> Create(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var storeTypeMapping = _storeTypeService.GetMappingByStoreId(currentStoreId);
                var category = model.ToEntity<Category>();
                category.StoreTypeId = storeTypeMapping?.StoreId ?? 0;
        ...
        public virtual async Task<IActionResult> Edit(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var prevBannerHeroPictureTopId = category.BannerHeroPictureTopId;

                var prevBannerHeroPictureLowerId = category.BannerHeroPictureLowerId;
                ...
                if (prevBannerHeroPictureTopId > 0 && prevBannerHeroPictureTopId != category.BannerHeroPictureTopId)
                {
                    var prevBannerPicture = _pictureService.GetPictureById(prevBannerHeroPictureTopId);
                    if (prevBannerPicture != null)
                        _pictureService.DeletePicture(prevBannerPicture);
                }

                if (prevBannerHeroPictureLowerId > 0 && prevBannerHeroPictureLowerId != category.BannerHeroPictureLowerId)
                {
                    var prevBannerPictureLower = _pictureService.GetPictureById(prevBannerHeroPictureLowerId);
                    if (prevBannerPictureLower != null)
                        _pictureService.DeletePicture(prevBannerPictureLower);
                }
                //update picture seo file name
                await UpdatePictureSeoNamesAsync(category);
        ...
        public virtual async Task <IActionResult> DeleteAll()
        {
            ...
            var categories = _categoryService.GetAllCategoriesAsync(storeId: _storeContext.ActiveStoreScopeConfiguration, showHidden: true)
                .Where(p => _workContext.CurrentVendor == null)
                .ToList();

            _categoryService.DeleteCategoriesAsync(categories);
        ...
        #region Promoted Products

        [HttpPost]
        public virtual IActionResult PromotedProductList(CategoryPromotedProductSearchModel searchModel)
        {
            ...
            //prepare model
            var model = _categoryModelFactory.PrepareCategoryPromotedProductListModel(searchModel, category);

            return Json(model);
        }

        public virtual IActionResult PromotedProductUpdate(CategoryPromotedProductModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();

            //try to get a promoted product category with the specified id
            var promotedProductCategory = _categoryService.GetPromotedProductCategoryById(model.Id)
                ?? throw new ArgumentException("No promoted product category mapping found with the specified id");

            //fill entity from product
            promotedProductCategory = model.ToEntity(promotedProductCategory);
            _categoryService.UpdatePromotedProductCategory(promotedProductCategory);

            return new NullJsonResult();
        }

        public virtual IActionResult PromotedProductDelete(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();

            //try to get a promoted product category with the specified id
            var promotedProductCategory = _categoryService.GetPromotedProductCategoryById(id)
                ?? throw new ArgumentException("No promoted product category mapping found with the specified id", nameof(id));

            _categoryService.DeletePromotedProductCategory(promotedProductCategory);

            return new NullJsonResult();
        }

        public virtual IActionResult PromotedProductAddPopup(int categoryId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();

            var existingPromotedProductCategories = _categoryService.GetPromotedProductCategoriesByCategoryId(categoryId, showHidden: true);
            if (existingPromotedProductCategories.Count >= 3)
                throw new ArgumentException("Max promoted product count is 3");

            //prepare model
            var model = _categoryModelFactory.PrepareAddPromotedProductToCategorySearchModel(new AddPromotedProductToCategorySearchModel());

            return View(model);
        }

        [HttpPost]
        public virtual IActionResult PromotedProductAddPopupList(AddPromotedProductToCategorySearchModel searchModel)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedDataTablesJson();

            //prepare model
            var model = _categoryModelFactory.PrepareAddPromotedProductToCategoryListModel(searchModel);
        ...
        [HttpPost]
        [FormValueRequired("save")]
        public virtual IActionResult PromotedProductAddPopup(AddPromotedProductToCategoryModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
            ...
            if (selectedProducts.Any())
            {
                var existingPromotedProductCategories = _categoryService.GetPromotedProductCategoriesByCategoryId(model.CategoryId, showHidden: true);
                foreach (var product in selectedProducts)
                {
                    //whether product category with such parameters already exists
                    if (_categoryService.FindPromotedProductCategory(existingPromotedProductCategories, product.Id, model.CategoryId) != null)
                        continue;

                    //insert the new product category mapping
                    _categoryService.InsertPromotedProductCategory(new PromotedProductCategory
                    {
                        CategoryId = model.CategoryId,
                        ProductId = product.Id,
                        IsFeaturedProduct = false,
            ...
            return View(new AddPromotedProductToCategorySearchModel());
        }
