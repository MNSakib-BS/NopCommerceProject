...
public partial class ArchDiscountController : BaseAdminController
{
    #region Fields
    ...
    public ArchDiscountController(CatalogSettings catalogSettings,
    ...
    protected virtual void SaveStoreMappings(Discount discount, DiscountModel model)
    {
        discount.LimitedToStores = model.SelectedStoreIds.Any();
        _discountService.UpdateDiscount(discount);

        var existingStoreMappings = _storeMappingService.GetStoreMappings(discount);
        ...
        foreach (var store in allStores)
        {
            if (model.SelectedStoreIds.Contains(store.Id))
            {
                ...
                if (existingStoreMappings.Count(sm => sm.StoreId == store.Id) == 0)
                    _storeMappingService.InsertStoreMapping(discount, store.Id);
            }
            else
            {
                //remove store
    ...
    public virtual async Task<IActionResult> Create(DiscountModel model, bool continueEditing)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            SaveStoreMappings(discount, model);
    ...
    #region Applied to customers 

    [HttpPost]
    public virtual IActionResult CustomerList(DiscountCustomerSearchModel searchModel)
    {
        ...
        //prepare model
        var model = _discountModelFactory.PrepareDiscountCustomerListModel(searchModel, discount);
    ...
    public virtual IActionResult CustomerDelete(int discountId, int customerId)
    {
        ...
        //remove discount
        if (_customerService.GetDiscountAppliedToCustomer(customer.Id, discount.Id) is DiscountCustomerMapping mapping)
            _customerService.DeleteDiscountCustomerMapping(mapping);
    ...
    public virtual IActionResult CustomerAddPopup(int discountId)
    {
        if (!_permissionService.Authorize(StandardPermissionProvider.ManageDiscounts))
            return AccessDeniedView();

        //prepare model
        var model = _discountModelFactory.PrepareAddCustomerToDiscountSearchModel(new AddCustomerToDiscountSearchModel());

        return View(model);
    }

    [HttpPost]
    public virtual IActionResult CustomerAddPopupList(AddCustomerToDiscountSearchModel searchModel)
    {
        if (!_permissionService.Authorize(StandardPermissionProvider.ManageDiscounts))
            return AccessDeniedDataTablesJson();

        //prepare model
        var model = _discountModelFactory.PrepareAddCustomerToDiscountListModel(searchModel);
    ...
    [HttpPost]
    [FormValueRequired("save")]
    public virtual IActionResult CustomerAddPopup(AddCustomerToDiscountModel model)
    {
        if (!_permissionService.Authorize(StandardPermissionProvider.ManageDiscounts))
            return AccessDeniedView();
        ...
        foreach (var id in model.SelectedCustomerIds)
        {
            var customer = _customerService.GetCustomerById(id);
            if (customer == null)
                continue;

            if (_customerService.GetDiscountAppliedToCustomer(customer.Id, discount.Id) is null)
                _customerService.InsertDiscountCustomerMapping(new DiscountCustomerMapping { DiscountId = discount.Id, EntityId = customer.Id });
        ...
        return View(new AddCustomerToDiscountSearchModel());
    }
