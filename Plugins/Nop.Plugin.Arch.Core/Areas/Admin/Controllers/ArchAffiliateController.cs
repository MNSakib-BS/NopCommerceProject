...
{
    public partial class ArchAffiliateController : BaseAdminController
    {
        #region Fields
        ...
        public ArchAffiliateController(IAddressService addressService,
        ...
        protected virtual void SaveStoreMappings(Affiliate affiliate, AffiliateModel model)
        {
            affiliate.LimitedToStores = model.SelectedStoreIds.Any();
            _affiliateService.UpdateAffiliate(affiliate);

            var existingStoreMappings = _storeMappingService.GetStoreMappings(affiliate);
            ...
            foreach (var store in allStores)
            {
                if (model.SelectedStoreIds.Contains(store.Id))
                {
                    ...
                    if (existingStoreMappings.Count(sm => sm.StoreId == store.Id) == 0)
                        _storeMappingService.InsertStoreMapping(affiliate, store.Id);
                }
                else
                {
                    //remove store
        ...
        public virtual async Task<IActionResult> Create(AffiliateModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //Stores
                SaveStoreMappings(affiliate, model);
