...
public partial class ArchOrderController : BaseAdminController
{
    #region Fields
    ...
    public ArchOrderController(IAddressService addressService,
    ...
    public virtual IActionResult OrderListByStatus(int orderStatusId)
    {
        List<int> orderStatuses = new List<int>();
        orderStatuses.Add(Convert.ToInt32(orderStatusId));

        return RedirectToAction("List", new { orderStatuses = orderStatuses });
    }
    [HttpPost]
    ...
    private void ConfirmArch(Order order, Customer customer, int storeId)
    {
        var resanehlabService = EngineContext.Current.Resolve<IResanehlabService>();
        var archInvoiceService = EngineContext.Current.Resolve<IArchInvoiceService>();
        var deliveryDateTime = resanehlabService.GetEstimatedDateForOrder(order.Id);
        archInvoiceService.ConfirmArch(order, customer, storeId, deliveryDateTime);
    }
    [HttpPost, ActionName("Edit")]
    ...
    [HttpPost, ActionName("Edit")]
    [FormValueRequired("btnSaveShippingStatus")]
    public virtual IActionResult ChangeShippingStatus(int id, OrderModel model)
    {
        ...
        try
        {
            ...
            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopOrderDefaults.AdminShippingStatusUpdateCacheKey, order.Id).Key;
            _genericAttributeService.SaveAttribute(order, cacheKey, order.ShippingStatusId, order.StoreId);
            ...
            {
                OrderId = order.Id,
                Note = $"Shipping status has been edited. New status: {_localizationService.GetLocalizedEnum(order.ShippingStatus)}",
                DisplayToCustomer = false,
                CreatedOnUtc = DateTime.UtcNow
            });

            var shipment = GetLatestShipment(order, order.ShippingStatus == ShippingStatus.ReturnedToStore);
    ...
    public virtual async Task<IActionResult> AddressEdit(OrderAddressModel model, IFormCollection form)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            var cacheKeyOrderNote = _cacheKeyService.PrepareKeyForDefaultCache(NopOrderDefaults.OrderNoteChangeTypeCacheKey, orderNote.Id).Key;
            _genericAttributeService.SaveAttribute(orderNote, cacheKeyOrderNote, (int)OrderNoteChangeType.ShippingAddressChange);
    ...
    private Shipment GetLatestShipment(Order order, bool isDelivered, bool isReturned = false)
    {
        return _shipmentService.GetShipmentsByOrderId(order.Id, shipped: true)
