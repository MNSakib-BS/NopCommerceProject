...
public partial class ArchProductController : BaseAdminController
{
    #region Fields
    ...
    public ArchProductController(IAclService aclService,
    ...
    public virtual async Task<IActionResult> EnsureCategory(int id)
    {
        if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageProducts))
            return AccessDeniedView();

        //try to get a product with the specified id
        var product = await _productService.GetProductByIdAsync(id, _storeContext.ActiveStoreScopeConfiguration);
        if (product == null || product.Deleted)
            return RedirectToAction("List");
        ...
        await _productService.EnsureProductDepartment(product, product.DepartmentNumberField, null, _storeContext.StoreTypeId);
    ...
    public virtual async Task<IActionResult> DeleteAll()
    {
        ...
        var inHoldingProducts = _productService.SearchProductsAsync(storeId: _storeContext.ActiveStoreScopeConfiguration,
                isInHolding: true,
                promotionGroupId: 0,
                showHidden: true)
            .Where(p => _workContext.CurrentVendor == null || p.VendorId == _workContext.CurrentVendor.Id)
            .ToList();

        _productService.DeleteProductsAsync(inHoldingProducts);

        var notInHoldingProducts = _productService.SearchProductsAsync(storeId: _storeContext.ActiveStoreScopeConfiguration,
                isInHolding: false,
                promotionGroupId: 0,
                showHidden: true)
            .Where(p => _workContext.CurrentVendor == null || p.VendorId == _workContext.CurrentVendor.Id)
        ...
        _productService.DeleteProductsAsync(notInHoldingProducts);
    ...
    public virtual IActionResult ProcessImages()
    {
        var pictureImportService = EngineContext.Current.Resolve<IPictureImportService>();
        pictureImportService.ImportPictures();
        return Ok();
    }
    #endregion
