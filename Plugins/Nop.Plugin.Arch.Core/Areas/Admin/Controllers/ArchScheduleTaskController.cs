...
public partial class ArchScheduleTaskController : BaseAdminController
{
    #region Fields
    ...
    public ArchScheduleTaskController(ICustomerActivityService customerActivityService,
    ...
    public virtual async Task <IActionResult> Stop(int id)
    {
        if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageScheduleTasks))
            return AccessDeniedView();
        ...
        {
            //try to get a schedule task with the specified id
            var scheduleTask = _scheduleTaskService.GetTaskByIdAsync(id)
                               ?? throw new ArgumentException("Schedule task cannot be loaded", nameof(id));

            //ensure that the task is enabled
            var task = new Task(scheduleTask);
            task.Stop(EngineContext.Current.Resolve<IStoreContext>().CurrentStore.Id);

            _notificationService.SuccessNotification(_localizationService.GetResource("Admin.System.ScheduleTasks.Stop.Done"));
        }
        catch (Exception exc)
        {
