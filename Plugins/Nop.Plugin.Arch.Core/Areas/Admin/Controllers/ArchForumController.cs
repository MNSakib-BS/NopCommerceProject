...
public partial class ArchForumController : BaseAdminController
{
    #region Fields
    ...
    public ArchForumController(IForumModelFactory forumModelFactory,
    ...
    protected virtual void SaveStoreMappings(Forum forum, ForumModel model)
    {
        forum.LimitedToStores = model.SelectedStoreIds.Any();
        _forumService.UpdateForum(forum);

        var existingStoreMappings = _storeMappingService.GetStoreMappings(forum);
        ...
        foreach (var store in allStores)
        {
            if (model.SelectedStoreIds.Contains(store.Id))
            {
                ...
                if (existingStoreMappings.Count(sm => sm.StoreId == store.Id) == 0)
                    _storeMappingService.InsertStoreMapping(forum, store.Id);
            }
            else
            {
                //remove store
    ...
    protected virtual void SaveStoreMappings(ForumGroup forumGroup, ForumGroupModel model)
    {
        forumGroup.LimitedToStores = model.SelectedStoreIds.Any();
        _forumService.UpdateForumGroup(forumGroup);

        var existingStoreMappings = _storeMappingService.GetStoreMappings(forumGroup);
        var allStores = _storeService.GetAllStores();
        foreach (var store in allStores)
        {
            if (model.SelectedStoreIds.Contains(store.Id))
            {
                //new store
                if (existingStoreMappings.Count(sm => sm.StoreId == store.Id) == 0)
                    _storeMappingService.InsertStoreMapping(forumGroup, store.Id);
            }
            else
            {
                //remove store
