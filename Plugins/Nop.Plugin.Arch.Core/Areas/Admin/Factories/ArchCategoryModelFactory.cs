...
/// </summary>
public partial class ArchCategoryModelFactory : ICategoryModelFactory
{
    #region Fields
    ...
    public ArchCategoryModelFactory(CatalogSettings catalogSettings,
    ...
    /// <summary>
    /// Prepare category promoted product search model
    /// </summary>
    /// <param name="searchModel">Category promoted product search model</param>
    /// <param name="category">Category</param>
    /// <returns>Category promoted product search model</returns>
    protected override CategoryPromotedProductSearchModel PrepareCategoryPromotedProductSearchModel(CategoryPromotedProductSearchModel searchModel, Category category)
    {
        if (searchModel == null)
    ...
    public override CategoryPromotedProductListModel PrepareCategoryPromotedProductListModel(CategoryPromotedProductSearchModel searchModel, Category category)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
        ...
        //get product categories
        var productCategories = _categoryService.GetPromotedProductCategoriesByCategoryId(category.Id,
            showHidden: true,
            pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);

        //prepare grid model
        var model = new CategoryPromotedProductListModel().PrepareToGrid(searchModel, productCategories, () =>
        {
            return productCategories.Select(productCategory =>
            {
                //fill in model values from the entity
                var categoryProductModel = productCategory.ToModel<CategoryPromotedProductModel>();
    ...
    /// <summary>
    /// Prepare promoted product search model to add to the category
    /// </summary>
    /// <param name="searchModel">Promoted product search model to add to the category</param>
    /// <returns>Promoted product search model to add to the category</returns>
    public override AddPromotedProductToCategorySearchModel PrepareAddPromotedProductToCategorySearchModel(AddPromotedProductToCategorySearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
    ...
    /// <summary>
    /// Prepare paged promoted product list model to add to the category
    /// </summary>
    /// <param name="searchModel">Promoted product search model to add to the category</param>
    /// <returns>Promoted product list model to add to the category</returns>
    public override AddPromotedProductToCategoryListModel PrepareAddPromotedProductToCategoryListModel(AddPromotedProductToCategorySearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
        ...
        //prepare grid model
        var model = new AddPromotedProductToCategoryListModel().PrepareToGrid(searchModel, products, () =>
        {
