...
/// </summary>
public partial class ArchBaseAdminModelFactory : IBaseAdminModelFactory
{
    #region Fields
    ...
    public ArchBaseAdminModelFactory(ICategoryService categoryService,
        ...
        IVendorService vendorService)
    {
        ...
        _recommendedListService = EngineContext.Current.Resolve<IRecommendedListService>();
        _blogService = EngineContext.Current.Resolve<IBlogService>();
        _promotionGroupService = EngineContext.Current.Resolve<IPromotionGroupService>();
        _promotionGroupTemplateService = EngineContext.Current.Resolve<IPromotionGroupTemplateService>();
    }
    ...
    /// <returns>PromotionGroup list</returns>
    protected override List<SelectListItem> GetPromotionGroupList(bool showHidden = true)
    {
        var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.PromotionGroupsListKey, showHidden);
        var listItems = _staticCacheManager.Get(cacheKey, () =>
        {
            var promotionGroups = _promotionGroupService.GetAllPromotionGroups(showHidden: showHidden);
            return promotionGroups.Select(m => new SelectListItem
            {
                Text = m.Name,
                Value = m.Id.ToString()
            });
    ...
    /// <summary>
    /// Get recommended list list
    /// </summary>
    /// <returns>RecommendedList list</returns>
    protected override List<SelectListItem> GetRecommendedListList()
    {
        var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.RecommendedListsListKey);
        var listItems = _staticCacheManager.Get(cacheKey, () =>
        {
            var recommendedLists = _recommendedListService.GetAllRecommendedLists();
            return recommendedLists.Select(m => new SelectListItem
            {
                Text = m.Name,
                Value = m.Id.ToString()
            });
    ...
    /// <summary>
    /// Get publish blog list
    /// </summary>
    /// <param name="showHidden">A value indicating whether to show hidden records</param>
    /// <returns>Vendor list</returns>
    protected override List<SelectListItem> GetBlogList(bool showHidden = true)
    {
        var blogs = _blogService.GetAllBlogPosts(showHidden: showHidden, storeId: _storeContext.ActiveStoreScopeConfiguration);
        var listItems = blogs.Select(v => new SelectListItem
        {
            Text = v.Title,
            Value = v.Id.ToString()
        });
    ...
    /// <summary>
    /// /// Prepare available promotion groups
    /// </summary>
    /// <param name="items">PromotionGroup items</param>
    /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
    /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
    public override void PreparePromotionGroups(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
    {
        if (items == null)
            throw new ArgumentNullException(nameof(items));

        //prepare available promotion groupss
        var availablePromotionGroupItems = GetPromotionGroupList();
        foreach (var promotionGroupItem in availablePromotionGroupItems)
        {
            items.Add(promotionGroupItem);
        }
    ...
    /// <summary>
    /// Prepare available recommended lists
    /// </summary>
    /// <param name="items">RecommendedList items</param>
    /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
    /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
    public override void PrepareRecommendedLists(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
    {
        if (items == null)
            throw new ArgumentNullException(nameof(items));

        //prepare available promotion groupss
        var availableRecommendedListItems = GetRecommendedListList();
        foreach (var recommendedListItem in availableRecommendedListItems)
        {
            items.Add(recommendedListItem);
        }
