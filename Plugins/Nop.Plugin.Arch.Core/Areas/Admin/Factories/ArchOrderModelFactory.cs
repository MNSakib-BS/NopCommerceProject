...
public partial class ArchOrderModelFactory : IOrderModelFactory
{
    ...
    /// <summary>
    /// Prepare order invoice item models
    /// </summary>
    /// <param name="models">List of order invoice item models</param>
    /// <param name="order">Order</param>
    protected override void PrepareOrderInvoiceItemModels(IList<ArchInvoiceItemModel> models, Order order)
    {
        if (models == null)
        ...
        var archInvoiceItems = _adminArchApi.EnsureInvoice(order.TransactionTrackingNumber, order.StoreId);
        foreach (var archInvoiceItem in archInvoiceItems)
        {
            ...
            var invoiceItemModel = new ArchInvoiceItemModel
            {
            ...
            var orderItem = _orderService.GetOrderItems(order.Id, vendorId: _workContext.CurrentVendor?.Id ?? 0)
            ...
            invoiceItemModel.PictureThumbnailUrl = _pictureService.GetPictureUrl(ref orderItemPicture, 75);

            models.Add(invoiceItemModel);
        }
    }
    ...
    protected virtual async Task PrepareOrderModelTotalsAsync(OrderModel model, Order order)
    {
        ...
        if (order.CustomerWalletDeduction.HasValue)
        {
            model.CustomerWalletDeduction = _priceFormatter.FormatPrice(order.CustomerWalletDeduction.Value, true, false);
        }

        //shopping bag
        if (order.ShoppingBagQuantity > 0 && order.ShoppingBagTotalCost > 0)
        {
            model.ShoppingBagQuantity = order.ShoppingBagQuantity;
            model.ShoppingBagTotalCost = _priceFormatter.FormatPrice(order.ShoppingBagTotalCost, true, false);
        }

        //paid on account
        if (order.PaidOnAccount.HasValue)
        {
            model.PaidOnAccount = _priceFormatter.FormatPrice(order.PaidOnAccount.Value, true, false);
        }

        var otpCacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopOrderDefaults.OrderOneTimePinCacheKey, order.Id).Key;
        model.OneTimePin = _genericAttributeService.GetAttribute<string>(order, otpCacheKey, order.StoreId);
    ...
    public override PaymentReconReportListModel PreparePaymentReconListModel(PaymentReconReportSearchModel searchModel)
    {
        //get report
        var report = _orderReportService.PaymentReconReport(_storeContext.ActiveStoreScopeConfiguration);

        var pagedList = new PagedList<PaymentReconReportLine>(report, searchModel.Page - 1, searchModel.PageSize);

        //prepare list model
        var model = new PaymentReconReportListModel().PrepareToGrid(searchModel, pagedList, () =>
        {
            //fill in model values from the entity
            return pagedList.Select(reportItem => new PaymentReconReportModel
            {
                OrderNumber = reportItem.OrderNumber,
                OrderTotal = _priceFormatter.FormatPrice(reportItem.OrderTotal, true, false),
                CustomerWalletDeduction = reportItem.CustomerWalletDeduction.HasValue && reportItem.CustomerWalletDeduction > 0 ? _priceFormatter.FormatPrice(reportItem.CustomerWalletDeduction.Value, true, false) : "-",
                PaidOnAccount = reportItem.PaidOnAccount.HasValue && reportItem.PaidOnAccount > 0 ? _priceFormatter.FormatPrice(reportItem.PaidOnAccount.Value, true, false) : "-",
                PaymentGatewayAmount = reportItem.PaymentGatewayAmount > 0 ? _priceFormatter.FormatPrice(reportItem.PaymentGatewayAmount, true, false) : "-",
                PaymentGateway = !string.IsNullOrWhiteSpace(reportItem.PaymentGateway) ? reportItem.PaymentGateway : "-",
                InvoiceAmount = reportItem.InvoiceAmount.HasValue ? _priceFormatter.FormatPrice(reportItem.InvoiceAmount.Value, true, false) : "-",
                WalletAmount = reportItem.WalletAmount.HasValue && reportItem.WalletAmount > 0 ? _priceFormatter.FormatPrice(reportItem.WalletAmount.Value, true, false) : "-",
                WalletTransactionType = reportItem.WalletTransactionType.HasValue ? _localizationService.GetLocalizedEnum(reportItem.WalletTransactionType.Value) : "-",
                CashbackStatus = reportItem.CashbackStatus.HasValue ? _localizationService.GetLocalizedEnum(reportItem.CashbackStatus.Value) : "-",
                IsDebtor = reportItem.IsDebtor ? "Yes" : "No"
            });
        });
