...
/// </summary>
public partial class ArchDiscountModelFactory : IDiscountModelFactory
{
    #region Fields
    ...
    public ArchDiscountModelFactory(CurrencySettings currencySettings,
    ...
    /// <summary>
    /// Prepare discount customer search model
    /// </summary>
    /// <param name="searchModel">Discount customer search model</param>
    /// <param name="discount">Discount</param>
    /// <returns>Discount customer search model</returns>
    protected override DiscountCustomerSearchModel PrepareDiscountCustomerSearchModel(DiscountCustomerSearchModel searchModel, Discount discount)
    {
        if (searchModel == null)
    ...
    /// <summary>
    /// Prepare paged discount customer list model
    /// </summary>
    /// <param name="searchModel">Discount customer search model</param>
    /// <param name="discount">Discount</param>
    /// <returns>Discount customer list model</returns>
    public override DiscountCustomerListModel PrepareDiscountCustomerListModel(DiscountCustomerSearchModel searchModel, Discount discount)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
        ...
        //get customer with applied discount
        var discountCustomers = _customerService.GetCustomersByAppliedDiscount(discountId: discount.Id,
            showHidden: false,
            pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);

        //prepare grid model
        var model = new DiscountCustomerListModel().PrepareToGrid(searchModel, discountCustomers, () =>
        {
            //fill in model values from the entity
            return discountCustomers.Select(customer =>
            {
                var discountCustomerModel = new DiscountCustomerModel()
                {
    ...
    public override AddCategoryToDiscountSearchModel PrepareAddCategoryToDiscountSearchModel(AddCategoryToDiscountSearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
    ...
    /// <summary>
    /// Prepare customer search model to add to the discount
    /// </summary>
    /// <param name="searchModel">Customer search model to add to the discount</param>
    /// <returns>Customer search model to add to the discount</returns>
    public override AddCustomerToDiscountSearchModel PrepareAddCustomerToDiscountSearchModel(AddCustomerToDiscountSearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
    ...
    /// <summary>
    /// Prepare paged customer list model to add to the discount
    /// </summary>
    /// <param name="searchModel">Customer search model to add to the discount</param>
    /// <returns>Customer list model to add to the discount</returns>
    public override AddCustomerToDiscountListModel PrepareAddCustomerToDiscountListModel(AddCustomerToDiscountSearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));

        //get customers
        var customers = _customerService.GetAllCustomers(username: searchModel.SearchCustomerName,
            pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize, discounts: true);

        //prepare grid model
        var model = new AddCustomerToDiscountListModel().PrepareToGrid(searchModel, customers, () =>
        {
