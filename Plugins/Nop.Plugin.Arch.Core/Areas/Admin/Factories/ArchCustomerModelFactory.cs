...
namespace Nop.Plugin.Arch.Core.Areas.Admin.Factories;

/// <summary>
/// Represents the customer model factory implementation
/// </summary>
public partial class ArchCustomerModelFactory : ICustomerModelFactory
{
    #region Fields
    ...
    public ArchCustomerModelFactory(AddressSettings addressSettings,
    ...
    /// <summary>
    /// Prepare customer wallet search model
    /// </summary>
    /// <param name="searchModel">Customer wallet search model</param>
    /// <param name="customer">Customer</param>
    /// <returns>Customer wallet search model</returns>
    protected override CustomerWalletSearchModel PrepareCustomerWalletSearchModel(CustomerWalletSearchModel searchModel, Customer customer)
    {
        if (searchModel == null)
    ...
    /// <summary>
    /// Prepare customer wallet transaction search model
    /// </summary>
    /// <param name="searchModel">Customer wallet transaction search model</param>
    /// <returns>Customer wallet transaction search model</returns>
    public override CustomerWalletTransactionSearchModel PrepareCustomerWalletTransactionSearchModel(CustomerWalletTransactionSearchModel searchModel,
        CustomerWallet customerWallet)
    {
        ...
        if (customerWallet == null)
            throw new ArgumentNullException(nameof(customerWallet));

        searchModel.CustomerWalletId = customerWallet.Id;
    ...
    /// <summary>
    /// Prepare customer wallet model
    /// </summary>
    /// <param name="model">Customer wallet model</param>
    /// <param name="customer">Customer wallet</param>
    /// <returns>Customer wallet model</returns>
    public override CustomerWalletModel PrepareCustomerWalletModel(CustomerWalletModel model, CustomerWallet customerWallet)
    {
        if (customerWallet != null)
        {
            var customer = _customerService.GetCustomerById(customerWallet.CustomerId);
            var customerModel = PrepareCustomerModel(new CustomerModel(), customer);

            //fill in model values from the entity
            model ??= new CustomerWalletModel();

            var amountExcludingCashback = _customerWalletService.GetAmountExcludingCashback(customerWallet, _storeContext.ActiveStoreScopeConfiguration);

            model.Id = customerWallet.Id;
            model.CustomerId = customer.Id;
            model.CustomerName = $"{customerModel.FirstName} {customerModel.LastName}";
            model.Amount = _priceFormatter.FormatPrice(customerWallet.Amount, true, false);
            model.AmountExcludingCashback = _priceFormatter.FormatPrice(amountExcludingCashback, true, false);
            model.CashbackTotal = _priceFormatter.FormatPrice(customerWallet.Amount - amountExcludingCashback, true, false);
            model.CreatedOn = _dateTimeHelper.ConvertToUserTime(customerWallet.CreatedDateUtc, DateTimeKind.Utc);

            //prepare available stores
            _storeMappingSupportedModelFactory.PrepareModelStores(model, customerWallet, false);

            var storeId = model.SelectedStoreIds.FirstOrDefault();
            model.StoreName = _storeService.GetStoreById(storeId)?.Name ?? "Unknown";
        }
    ...
    /// <param name="searchModel">Customer wallet search model</param>
    /// <returns>Customer wallet search model</returns>
    public override CustomerWalletSearchModel PrepareCustomerWalletSearchModel(CustomerWalletSearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));

        searchModel.StoreId = _storeContext.ActiveStoreScopeConfiguration;
    ...
    /// <summary>
    /// Prepare paged customer wallet list model
    /// </summary>
    /// <param name="searchModel">Customer wallet search model</param>
    /// <param name="customer">Customer</param>
    /// <returns>Customer wallet list model</returns>
    public override CustomerWalletTransactionListModel PrepareCustomerWalletTransactionListModel(CustomerWalletTransactionSearchModel searchModel,
        CustomerWallet customerWallet)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
        ...
        var transactionTypeIds = (searchModel.TransactionTypeIds?.Contains(0) ?? true) ? null : searchModel.TransactionTypeIds.ToList();

        //get customer wallet transactions
        var walletTransactions = _customerWalletService.SearchCustomerWalletTransactions(customerWallet.Id,
            paymentStatusIds,
            transactionTypeIds,
        ...
        //prepare list model
        var model = new CustomerWalletTransactionListModel().PrepareToGrid(searchModel, walletTransactions, () =>
        {
            return walletTransactions.Select(walletTransaction =>
            {
                //fill in model values from the entity
                var walletTransactionModel = walletTransaction.ToModel<CustomerWalletTransactionModel>();

                //convert dates to the user time
                walletTransactionModel.CreatedOn = _dateTimeHelper.ConvertToUserTime(walletTransaction.CreatedDateUtc, DateTimeKind.Utc);

                var storeMapping = _storeMappingService.GetCustomerWalletStoreMappings(walletTransaction).FirstOrDefault();

                //fill in additional values (not existing in the entity)
                walletTransactionModel.LastModifiedTicks = walletTransaction.ModifiedDateUtc.Ticks.ToString();
                walletTransactionModel.StoreName = _storeService.GetStoreById(storeMapping.StoreId)?.Name ?? "Unknown";
                walletTransactionModel.Amount = _priceFormatter.FormatPrice(walletTransaction.Amount, true, false);
                walletTransactionModel.CashbackStatus = _localizationService.GetLocalizedEnum(walletTransaction.CashbackStatus);
                walletTransactionModel.CashbackStatusId = walletTransaction.CashbackStatusId;
                walletTransactionModel.TransactionType = _localizationService.GetLocalizedEnum(walletTransaction.WalletTransactionType);
                walletTransactionModel.TransactionTypeId = walletTransaction.WalletTransactionTypeId;
    ...
    /// <param name="customer">Customer</param>
    /// <returns>Customer wallet list model</returns>
    public override CustomerWalletListModel PrepareCustomerWalletListModel(CustomerWalletSearchModel searchModel, Customer customer)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));

        //get customer wallets
        var wallets = _customerWalletService.SearchCustomerWallets(customer?.Id,
            searchModel.StartDate,
            searchModel.EndDate,
            searchModel.StoreId,
            searchModel.Page - 1,
        ...
        //prepare list model
        var model = new CustomerWalletListModel().PrepareToGrid(searchModel, wallets, () =>
        {
            return wallets.Select(wallet =>
            {
                //fill in model values from the entity
                var walletModel = wallet.ToModel<CustomerWalletModel>();

                //convert dates to the user time
                walletModel.CreatedOn = _dateTimeHelper.ConvertToUserTime(wallet.CreatedDateUtc, DateTimeKind.Utc);

                var storeMapping = _storeMappingService.GetCustomerWalletStoreMappings(wallet).FirstOrDefault();

                var amountExcludingCashback = _customerWalletService.GetAmountExcludingCashback(wallet, _storeContext.ActiveStoreScopeConfiguration);

                //fill in additional values (not existing in the entity)
                walletModel.StoreName = storeMapping == null ? "Unknown" : _storeService.GetStoreById(storeMapping.StoreId)?.Name ?? "Unknown";
                walletModel.Amount = _priceFormatter.FormatPrice(wallet.Amount, true, false);
                walletModel.AmountExcludingCashback = _priceFormatter.FormatPrice(amountExcludingCashback, true, false);
                walletModel.CustomerId = customer.Id;
                walletModel.CustomerName = $"{customerModel.FirstName} {customerModel.LastName}";
    ...
    /// <summary>
    /// Prepare debtor customer search model
    /// </summary>
    /// <param name="searchModel">Debtor customer search model</param>
    /// <returns>Debtor customer search model</returns>
    public override DebtorCustomerSearchModel PrepareDebtorCustomerSearchModel(DebtorCustomerSearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
    ...
    /// <summary>
    /// Prepare paged debtor customer list model
    /// </summary>
    /// <param name="searchModel">Debtor customer search model</param>
    /// <returns>Debtor customer list model</returns>
    public override DebtorCustomerListModel PrepareDebtorCustomerListModel(DebtorCustomerSearchModel searchModel)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
        ...
        var customers = _customerService.GetDebtorCustomers(searchModel.Page - 1, pageSize: searchModel.PageSize, storeId: _storeContext.ActiveStoreScopeConfiguration);

        //prepare list model
        var model = new DebtorCustomerListModel().PrepareToGrid(searchModel, customers, () =>
        {
            return customers.Select(customer =>
            {
                var debtorCustomerModel = customer.ToModel<DebtorCustomerModel>();

                debtorCustomerModel.DebtorNumber = customer.DebtorNumber(_storeContext.ActiveStoreScopeConfiguration);
                debtorCustomerModel.AccountNameField = customer.DebtorAccountName(_storeContext.ActiveStoreScopeConfiguration);
                debtorCustomerModel.DebtorTypeField = customer.DebtorType(_storeContext.ActiveStoreScopeConfiguration).ToString();
                debtorCustomerModel.StatusField = customer.DebtorAccountStatus(_storeContext.ActiveStoreScopeConfiguration).ToString();
                debtorCustomerModel.ErrorCodeField = customer.DebtorErrorCode(_storeContext.ActiveStoreScopeConfiguration);

                return debtorCustomerModel;
            });
        });
    ...
    /// <summary>
    /// Prepare paged customer picture list model
    /// </summary>
    /// <param name="searchModel">Customer picture search model</param>
    /// <param name="customer">Customer</param>
    /// <returns>Customer picture list model</returns>
    public override CustomerPictureListModel PrepareCustomerPictureListModel(CustomerPictureSearchModel searchModel, Customer customer)
    {
        if (searchModel == null)
            throw new ArgumentNullException(nameof(searchModel));
        ...
        var customerPictures = _customerService.GetCustomerPicturesByCustomerId(customer.Id).ToPagedList(searchModel);

        //prepare grid model
        var model = new CustomerPictureListModel().PrepareToGrid(searchModel, customerPictures, () =>
        {
            return customerPictures.Select(customerPicture =>
            {
                //fill in model values from the entity
                var customerPictureModel = customerPicture.ToModel<CustomerPictureModel>();

                //fill in additional values (not existing in the entity)
                var picture = _pictureService.GetPictureById(customerPicture.PictureId)
                              ?? throw new Exception("Picture cannot be loaded");

                customerPictureModel.PictureUrl = _pictureService.GetPictureUrl(ref picture);
